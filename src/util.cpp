...
const char * const BITCOIN_CONF_FILENAME = "gozer.conf";
const char * const BITCOIN_PID_FILENAME = "gozerd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "gozer" is a composite category enabling all Gozer-related debug output
                if(ptrCategory->count(std::string("gozer"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GozerCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GozerCore
    // Mac: ~/Library/Application Support/GozerCore
    // Unix: ~/.gozercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GozerCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GozerCore";
#else
    // Unix
    return pathRet / ".gozercore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty gozer.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override gozer.conf
