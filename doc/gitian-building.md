...
*Setup instructions for a Gitian build of Gozer Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Gozer
...
- [Building Gozer Core](#building-gozer-core)
...
Clone the git repositories for Gozer Core and Gitian.
...
in the Gozer Core repository under 'Fetch and create inputs' to install sources which require
...
To build Gozer Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Gozer Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/gozer/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/gozerpay/gozer
...
For example:
```bash
URL=https://github.com/crowning-/gozer.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit gozer=${COMMIT} --url gozer=${URL} ../gozer/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit gozer=${COMMIT} --url gozer=${URL} ../gozer/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit gozer=${COMMIT} --url gozer=${URL} ../gozer/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the gozer git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../gozer/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/gozer
SIGPATH=/some/root/path/gozer-detached-sigs

./bin/gbuild --url gozer=${BTCPATH},signature=${SIGPATH} ../gozer/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/gozer-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/gozer-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/gozer-osx-build.assert
```
...
[gozer/gitian.sigs](https://github.com/gozerpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
